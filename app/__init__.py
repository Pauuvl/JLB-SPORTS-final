...
def create_app(config_object=None):
    ...
    # Endpoint liviano para servicios de "ping" externos (UptimeRobot, etc.)
    # que evitan que el plan gratuito de Render duerma la app por inactividad.
    @app.route('/healthz')
    def healthz():
    ...
    # ------------------------------------------------------------
    # Crear administrador automáticamente si las variables de entorno
    # ADMIN_USERNAME / ADMIN_PASSWORD están configuradas en Render.
    # ------------------------------------------------------------
        ...
        if admin_username and admin_password:
            try:
                ...
                if admin is None:
                    ...
                    db.session.commit()
                    app.logger.info('Administrador "%s" creado automáticamente.', admin_username)
            except Exception as exc:  # nunca debe tumbar el arranque de la app
                app.logger.warning('No se pudo crear el admin automático: %s', exc)
...
def _register_security_headers(app):
    """Cabeceras básicas de seguridad para producción."""
